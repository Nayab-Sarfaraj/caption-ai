import type {
  ...
  TranscriptWord,
  TranscriptSegment,
} from "@/src/types/transcript.types";
...
class DeepgramProvider implements TranscriptionProvider {
  async transcribe(audioUrl: string): Promise<Transcript> {
    ...
    if (!alternative?.words?.length) {
      throw new Error(
        "No transcription results — audio may be silent or invalid",
      );
    }
...
class WhisperProvider implements TranscriptionProvider {
  async transcribe(): Promise<Transcript> {
    throw new Error(
      "Whisper provider not implemented — set TRANSCRIPTION_PROVIDER=deepgram",
    );
  }
}
...
export function getTranscriptionProvider(): TranscriptionProvider {
  if (process.env.TRANSCRIPTION_PROVIDER === "whisper")

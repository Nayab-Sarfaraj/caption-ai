...
export function UploadDropzone({
  ...
  const [batchMode, _setBatchMode] = useState(false);
  ...
  const onCaptionDrop = useCallback((f: File[]) => {
    if (f[0]) setCaptionFile(f[0]);
  }, []);
        ...
        {batchMode ? (
          videoFiles.length > 0 ? (
            <div className="w-full py-6 px-4 space-y-1.5 max-h-[200px] overflow-y-auto text-left">
              {videoFiles.map((f, i) => (
                ...
                >
                  ...
                  <span className="text-xs text-[var(--ink-dim)] shrink-0 ml-2">
            ...
            <div className="py-8">
              ...
              <p className="text-sm text-[var(--ink)]">
                Drop up to {MAX_BATCH_FILES} videos
              </p>
              <p className="text-xs text-[var(--mute)] mt-1.5">
                MP4 or MOV · 500MB max each
              </p>
            </div>
          )
        ) : videoFile ? (
          <div className="flex items-center gap-3 w-full px-5 py-6 text-left">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm text-[var(--ink)] truncate font-medium">
                {videoFile.name}
              </p>
              <p className="text-xs text-[var(--ink-dim)] mt-0.5">
                {(videoFile.size / 1024 / 1024).toFixed(1)} MB ·{" "}
                {videoFile.type === "video/mp4" ? "MP4" : "MOV"}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="py-8">
            ...
            <p className="text-sm text-[var(--ink)]">
              Import or drag your video
            </p>
            <p className="text-xs text-[var(--mute)] mt-1.5">
              MP4 or MOV · 500MB max · 10:00 max
            </p>
          </div>
        )}
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="px-3 py-2.5 bg-[var(--panel)]">
                    ...
                    <p className="text-[11px] text-[var(--mute)] leading-tight mt-1 line-clamp-1">
                      {s.desc}
                    </p>
                  </div>
                </button>
      ...
      {!batchMode && (
        <div>
          ...
          >
            {showCaption
              ? "− Hide"
              : "+ Have an .srt or .vtt? Skip AI transcription"}
          </button>
          {showCaption && (
            <div
        ...
        <div className="space-y-1.5">
          <div className="h-[3px] w-full rounded-full bg-[var(--hair)] overflow-hidden">
            <div
              ...
              style={{
                width: `${step === "uploading" ? uploadProgress : 100}%`,
              }}
            />
          </div>
...
function getVideoDimensions(
  ...
  return new Promise((resolve, reject) => {
    ...
    video.onerror = () => {
      URL.revokeObjectURL(url);
      reject(new Error("Cannot read dimensions"));
    };
    video.src = url;
  });
}

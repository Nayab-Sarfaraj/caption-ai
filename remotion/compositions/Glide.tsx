...
// Words glide in horizontally from the right as they're spoken — a different
// motion axis from the pop/scale of the other styles.
export const Glide: React.FC<GlideProps> = ({
  transcript,
  videoSrc,
  activeColor = '#FACC15',
  textColor = '#FFFFFF',
  ...
  const glideDist = Math.round(fontSize * 0.9)
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: 'transparent' }}>
      ...
      {currentSegment && (
        <AbsoluteFill style={{ ...captionAnchorStyle(paddingBottom, posX, posY), paddingLeft: paddingH, paddingRight: paddingH, overflow: 'hidden' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35em', justifyContent: 'center', maxWidth }}>
            {visibleWords.map((word, i) => {
              const started = currentTime >= word.start
              const isCurrent = currentTime >= word.start && currentTime < word.end
              const elapsed = Math.max(0, frame - Math.floor(word.start * fps))
              const offset = started ? interpolate(elapsed, [0, 7], [glideDist, 0], { extrapolateRight: 'clamp' }) : glideDist
              const opacity = started ? interpolate(elapsed, [0, 6], [0, 1], { extrapolateRight: 'clamp' }) : 0
              return (
                <span
                  key={i}
                  style={{
